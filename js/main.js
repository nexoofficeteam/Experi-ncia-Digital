...
function nexoRenderizarProjetos() {
  ...
  grade.innerHTML = NEXO_CONFIG.projetos.map((projeto) => {
    ...
    const temUrl = Boolean(projeto.url);
      ...
      ? `<img class="card-projeto-capa-fundo" src="${projeto.imagem}" alt="" aria-hidden="true" loading="lazy">
         <img class="card-projeto-capa-imagem" src="${projeto.imagem}" alt="Print do site ${projeto.nome}" loading="lazy" width="640" height="400">`
